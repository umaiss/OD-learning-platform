...
async def generate_structured(
    ...
    schema_desc = _get_schema_description(schema)
    json_prompt = f"""You are a helpful assistant that returns ONLY valid JSON.
CRITICAL: Do not include any markdown formatting, code blocks, or explanatory text.
...
Remember:
- Arrays must contain the specified type (strings, not objects, unless specified)
- All required fields must be included
- Types must match exactly (strings, integers, arrays, objects)
- Do not add extra fields or nested structures unless specified in the schema"""
...
def _extract_json(text: str) -> dict:
    ...
    try:
        ...
        # Sometimes the LLM includes trailing commas or other issues
        # Try to clean it up
        try:
...
def _get_schema_description(schema: Type[BaseModel]) -> str:
    ...
    try:
        ...
        for field_name, field_info in properties.items():
            ...
            # Handle array types - check items
            if field_type == "array":
                items = field_info.get("items", {})
                item_type = items.get("type", "any")
                if item_type == "object":
                    # For object arrays, describe the object structure
                    item_props = items.get("properties", {})
                    item_desc = "{"
                    for prop_name, prop_info in item_props.items():
                        prop_type = prop_info.get("type", "any")
                        item_desc += f'"{prop_name}": {prop_type}, '
                    item_desc = item_desc.rstrip(", ") + "}"
                    field_type = f"array of {item_desc}"
                else:
                    field_type = f"array of {item_type}"
            
            # Handle object types - describe structure
            elif field_type == "object":
                obj_props = field_info.get("properties", {})
                obj_desc = "{"
                for prop_name, prop_info in obj_props.items():
                    prop_type = prop_info.get("type", "any")
                    obj_desc += f'"{prop_name}": {prop_type}, '
                obj_desc = obj_desc.rstrip(", ") + "}"
                field_type = obj_desc
